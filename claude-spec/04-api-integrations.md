...
**Composer lookup** (#890, #920). `chatgpt_sendMsg()` waits up to 15 s for the composer via `findPromptInput()` (a MutationObserver plus a 250 ms poll), trying `MZTA_PROMPT_SELECTORS` in order and taking only **visible** elements, so a hidden fallback `<textarea>` never wins over the visible contenteditable. Open shadow roots are searched too, with a bounded walk, on the poll ticks only. If the wait times out, it falls back to a non-visible `#prompt-textarea`, which is what the old single lookup would have found. A contenteditable gets the HTML nodes. A real `<textarea>` gets the prompt converted to plain text (`htmlToPlainText()`, one line per `<p>`), set through the native `value` setter and followed by a bubbling `input` event. When nothing is found, `logComposerDiagnostics()` always emits one `console.warn("[ThunderAI] Diagnostics: {...}")` line with structural facts only (URL without query, title, element counts, up to 5 candidate inputs, login/Cloudflare hints, UA). It **must never** include page text or the prompt, because users paste it on GitHub. On `-2` (composer not found), `doProceed()` returns right after showing the retry button. It does not enter `chatgpt_isIdle()`, so retries don't stack idle-wait loops. `doRetry()` re-sends `_customTextArray` when custom text was collected.
...
`processEmails()` wraps its whole body in `try/finally` so `taWorkingStatus.stopWorking()` always runs, and wraps each message in `try/catch`+`continue` so one failing message does not abort the batch. Inside an iteration, the add-tags and translate blocks catch their own failures (e.g. `getFull()` on a message a filter just moved), so one failing feature does not skip the others for that message (see [01-architecture.md](01-architecture.md#concurrent-drains-spam-filter-and-summarize-in-processemails)). The spam and summary drains get the same isolation from `runWithConcurrency()`, which catches and logs each target's failure (see [01-architecture.md](01-architecture.md#concurrent-drains-spam-filter-and-summarize-in-processemails)).
...
**Interaction with `mzta_specialCommand`:** v1 cancellation is checked *between* messages and between the targets of the spam and summary drains, so the in-flight worker prompts (up to the drain's concurrency cap) are allowed to finish first (bounded by `special_command_timeout`). There is no mid-request `dispose()` in v1; a future enhancement could register the active `mzta_specialCommand` with the controller and terminate its worker on cancel for an immediate abort.
