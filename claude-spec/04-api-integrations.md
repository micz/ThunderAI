...
**Two-Phase Navigation Pattern for Projects and Custom GPTs**:
Direct cold HTTP GET requests to ChatGPT Projects (`/g/g-p-XXXX/project`) or Custom GPTs (`/g/g-XXXX`) frequently fail React/Next.js hydration on `chatgpt.com`, crashing into the OpenAI ErrorBoundary ("Retry" / "Volver a intentar"). To ensure reliable navigation, ThunderAI implements a 2-phase navigation flow:
1. **Phase 1 (Cold Home Load)**: The initial popup window (`browser.windows.create`) is always created on the root home page (`https://chatgpt.com?call_id=...`). This reliably boots the session, authenticates the user, and hydrates the core client SPA bundle.
2. **Phase 2 (SPA Client Navigation)**: When `chatgpt_send` is dispatched with `target_path`, `chatgpt_sendMsg()` checks if the current pathname already includes the target. If not, it performs a client-side transition:
   - It extracts the target identifier `targetId` (`/g-[a-zA-Z0-9-]+/`) and searches the DOM for an existing sidebar navigation link (`a[href*="<targetId>"]`). If found, it invokes `.click()` to let Next.js handle the native route transition.
   - If no DOM link is found (e.g. sidebar collapsed or not yet rendered), it falls back to the HTML5 History API (`window.history.pushState(null, '', target_path)`) and dispatches a synthetic `PopStateEvent('popstate')`.
3. **Resilient Textarea Polling & Home-Page Fallback**:
   - Asynchronously polls for the route transition and `#prompt-textarea` to mount in the target view.
   - If the target view fails to mount the textarea within the timeout (e.g. target routing error or ErrorBoundary crash), ThunderAI falls back to the general conversation on the home page (`a[href="/"]` or History API), displays a prominent warning banner (`#mzta-fallback-warn`) with `chatgpt_custom_target_fallback_warning`, and injects the prompt into the available `#prompt-textarea` so user work is never lost.
   - If no textarea is found anywhere in the DOM, it logs an error and returns `-2`, presenting the Retry UI.
