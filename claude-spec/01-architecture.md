...
line and `.sel_info` becomes visible), it lands after an `await` on a storage read, and the
...
Regular preferences (feature flags, connection settings, `ollama_*`/`chatgpt_*`/etc., `reply_type`, `connection_type`, ...) live in `browser.storage.local`, keyed by `prefs_default` in `options/mzta-options-default.js`. They are read through the accessor module `js/mzta-prefs.js` (`mztaPrefs.getPref` / `getPrefs` / `getAllPrefs` / `setPref`), the single choke point for preference access — see [05-options.md](05-options.md#preference-access-jsmzta-prefsjs) for the API, the preserved missing-vs-`null` semantics and the handful of deliberate bypasses. The large-payload keys — `_custom_prompt`, `_default_prompts_properties`, `_special_prompts`, `_custom_placeholder`, `add_tags_exclusions` — live in the same area, where they went first, because `storage.sync` has a narrow storage quota (the one-time migrations in `js/mzta-utils.js`, `migrateCustomPromptsStorage()` / `migrateDefaultPromptsPropStorage()`, added for [#129](https://github.com/micz/ThunderAI/issues/129)).

**The preferences followed them out of `storage.sync` for the same quota reason.** The move is carried by `migratePrefsToLocal()` (`js/mzta-prefs-migration.js`), the first statement of `mzta-background.js`: it copies every key `storage.sync` still holds, skipping any already present in local (so it never reverts a later change), and **leaves the sync copy in place** so a downgrade still finds the user's settings. It then records `_prefs_migrated_from_sync` in `storage.local` and short-circuits on that marker at every subsequent startup, rather than re-reading both areas forever. Two consequences worth knowing: preferences **no longer sync across profiles or devices**, and every `storage.onChanged` listener that watches preferences gates on `areaName === 'local'` — a gate left on `'sync'` does not error, it simply stops firing. The area itself is decided once, by `PREFS_AREA` in `js/mzta-prefs.js`.
