...
| `diff_granularity` | `'words'` | Default comparison unit of the proofreading change picker: `'words'` or `'sentences'`. A prompt can override it (`''` on a prompt = inherit this), and the picker's own toolbar toggle changes it for the current review — see [07-diff-picker.md](07-diff-picker.md). Rendered as a `<select>` in the advanced section; needs an explicit entry in `restoreOptions()`'s `select-one` branch, since a select restoring to `''` would render blank. |
