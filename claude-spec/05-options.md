...
- **Split view**: a 3-line master row — lock glyph (read-only prompts) + name + type badge / id (mono) / 1-line text preview. Click or Enter selects; ArrowUp/Down move focus. The selected row has `--sel-bg` and a 3px `--sel-bar` left border. The menu/action value is intentionally not shown here.
...
- **Fields**: ID and Name inputs, the highlighted prompt textarea, Add to menu / Action selects, the **Menu position** button (edit mode only), and two disclosures for the per-prompt connection override:
...
- **Header buttons**:
  ...
  | Built-in | Restore default · Duplicate and edit |
  | Org / shadowed / inert | Duplicate and edit |
...
- **Restore default** puts back a built-in's `need_custom_text` from `getFactoryNeedCustomText()` (`js/mzta-prompts.js`), as a pending change.
