...
| `summarize_max_concurrency` | `1` | Maximum summaries generated at once by the summary drain of one `processEmails()` call (summarize on receive and the sender list). Independent of `spamfilter_max_concurrency`; overlapping calls can exceed it. Same `NaN` fallback. Number input (`min="1"`) in the summarize settings page, with its default wired into `restoreOptions()`. Does not affect the context-menu summarize flow. |
