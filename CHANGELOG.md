...
<h2>Version 5.1.0 - ??/??/2026</h2>
      <ul>
        ...
        <li><i>[Ollama API]</i> The "Enable thinking" option is now a reasoning level: model default, off, on, or one of the levels the selected model accepts, which are read from the model itself. An existing setting is converted automatically.</li>
        <li><i>[Ollama API]</i> Fix: choosing to not make the model think is now sent explicitly to the server. Some models reason anyway when the option is simply not sent, so the reasoning was still produced even with the thinking option disabled.</li>
        ...
        <li><i>[Ollama API]</i> The settings now adapt to the selected model: the thinking option is disabled when the model does not support it, and the maximum context size supported by the model is shown next to the context size field. If the server cannot be reached or does not report this information, all the options stay available.</li>
        <li><i>[Ollama API]</i> The connection test now checks the server version endpoint instead of the models list, so it can tell a server that cannot be reached (or a missing CORS configuration) from a server that is running but has no models pulled yet.</li>
        <li>...</li>
      </ul>
