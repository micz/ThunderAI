...
<h2>Version 5.1.0 - ??/??/2026</h2>
      <ul>
        ...
        <li><i>[OpenAI API]</i> The OpenAI options have been expanded and now adapt to the selected model: the <i>Max output tokens</i>, <i>Top P</i>, <i>Verbosity</i>, <i>Output format</i> (with a JSON schema), <i>Truncation</i>, <i>Prompt cache key</i>, <i>Service tier</i>, <i>Safety identifier</i> and <i>Include encrypted reasoning</i> advanced connection options have been added, also available per prompt, and the <i>Reasoning effort</i> option gained the levels the API accepts. Temperature and Top P are not accepted by the reasoning models, and the reasoning and verbosity options are ignored by the other ones, so they are disabled with an explanatory note when such a model is selected [<a href="https://github.com/micz/ThunderAI/issues/501">#501</a>].</li>
        <li><i>[OpenAI API]</i> Fix: the <i>Max output tokens</i> value was never sent to the server, so the length of the answer was always left to the API default.</li>
        <li>Added the <i>Force summary language</i> option in the Summarize settings, to always get the summary in a chosen language regardless of the language of the emails. If no language is entered, the default language set in the ThunderAI options is used [<a href="https://github.com/micz/ThunderAI/issues/907">#907</a>].</li>
        <li>Added the <i>Append a link to the original email</i> option in the Calendar Event and Task settings, to add a link to the source email at the end of the description of the created event or task. The link is added by ThunderAI and is never sent to the AI [<a href="https://github.com/micz/ThunderAI/issues/906">#906</a>].</li>
        <li><i>[All APIs]</i> When the AI server is temporarily unavailable (network error, timeout, overloaded model or HTTP 408, 429, 500, 502, 503, 504, 529 errors) the request is now automatically repeated up to 5 times, waiting longer before each attempt, or as long as the server asks [<a href="https://github.com/micz/ThunderAI/issues/879">#879</a>, <a href="https://github.com/micz/ThunderAI/issues/902">#902</a>].</li>
        <li><i>[All APIs]</i> The automatic processing of emails (add tags, spam filter, summarize, translate) now stops by itself when the AI provider's rate limit or quota is exceeded, reporting how many messages were processed and, when known, how long the provider asks to wait [<a href="https://github.com/micz/ThunderAI/issues/902">#902</a>].</li>
        <li><i>[All APIs]</i> The automatic features can now process more emails in parallel. The new <i>Emails processed in parallel</i> advanced option sets how many emails are handled at the same time (default 1) [<a href="https://github.com/micz/ThunderAI/issues/908">#908</a>, <a href="https://github.com/micz/ThunderAI/issues/909">#909</a>].</li>
        <li><i>[All APIs]</i> Fix: a failure reading a message during add tags or translate no longer skips the other automatic features for that message [<a href="https://github.com/micz/ThunderAI/issues/909">#909</a>].</li>
        <li>Fix: the <i>Force existing tags</i> option of the Add Tags settings now works: the list of the existing tags (or of the ones in the <i>Use only these tags</i> list that exist) is sent to the AI, the forced language is no longer requested in that case, and a tag proposed by the AI that does not exist is skipped instead of being created [<a href="https://github.com/micz/ThunderAI/issues/926">#926</a>].</li>
